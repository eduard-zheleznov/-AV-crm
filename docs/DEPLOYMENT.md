...
Релиз `1.12.27` после browser circuit-breaker нельзя выкатывать поверх работающего
worker. Сначала остановите run и убедитесь, что управляющие флаги запуска сняты.
Код и расширение `1.0.10` проверяются под оператором; автоматический production-run
...
1. `chrome://extensions`: расширение `1.0.10` включено, новое разрешение `debugger`
   принято, Console service worker без ошибок;
2. обычная страница Avito вручную отрисовывается и реагирует на прокрутку;
3. `avito-extension-test --max-clicks 1` проходит под оператором без CRM и очереди;
...
Для rollback `1.12.27` сначала остановите worker, затем откатите одновременно код
и unpacked extension к совместимой паре `1.12.26` / `1.0.9`. Не смешивайте версии:
