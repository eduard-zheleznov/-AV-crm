...
Релиз `1.12.44` с обязательным инкогнито нельзя выкатывать поверх работающего
worker. Сначала остановите run и убедитесь, что управляющие флаги запуска сняты.
Код и расширение `1.0.20` проверяются под оператором; автоматический production-run
...
1. `chrome://extensions`: расширение `1.0.20` включено, разрешения `scripting` и
...
Для rollback `1.12.44` сначала остановите worker, затем откатите одновременно код
и unpacked extension к совместимой паре `1.12.43` / `1.0.19` на commit
`c7b25d93c4f153ea411854979caeb394028a6a9a`. Не смешивайте версии:
