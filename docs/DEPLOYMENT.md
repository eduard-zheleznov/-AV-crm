...
Релиз `1.12.26` после browser circuit-breaker нельзя выкатывать поверх работающего
worker. Сначала остановите run и убедитесь, что управляющие флаги запуска сняты.
Код и расширение `1.0.9` проверяются под оператором; автоматический production-run
...
1. `chrome://extensions`: расширение `1.0.9` включено, Console service worker без ошибок;
...
Для rollback `1.12.26` сначала остановите worker, затем откатите одновременно код
и unpacked extension к совместимой паре `1.12.25` / `1.0.8`. Не смешивайте версии:
