...
Релиз `1.12.32` после browser circuit-breaker нельзя выкатывать поверх работающего
worker. Сначала остановите run и убедитесь, что управляющие флаги запуска сняты.
Код и расширение `1.0.14` проверяются под оператором; автоматический production-run
...
1. `chrome://extensions`: расширение `1.0.14` включено, разрешения `debugger` и
...
Для rollback `1.12.32` сначала остановите worker, затем откатите одновременно код
и unpacked extension к совместимой паре `1.12.31` / `1.0.13`. Не смешивайте версии:
