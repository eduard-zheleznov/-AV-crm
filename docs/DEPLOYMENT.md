...
Релиз `1.12.34` после browser circuit-breaker нельзя выкатывать поверх работающего
worker. Сначала остановите run и убедитесь, что управляющие флаги запуска сняты.
Код и расширение `1.0.16` проверяются под оператором; автоматический production-run
...
1. `chrome://extensions`: расширение `1.0.16` включено, разрешение `scripting`
   принято, разрешения `debugger` нет, Console service worker без ошибок;
...
Для rollback `1.12.34` сначала остановите worker, затем откатите одновременно код
и unpacked extension к совместимой паре `1.12.33` / `1.0.15`. Не смешивайте версии:
